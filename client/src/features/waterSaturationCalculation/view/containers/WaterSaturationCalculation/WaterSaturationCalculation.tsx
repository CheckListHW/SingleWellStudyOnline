...
import './WaterSaturationCalculation.scss';
...
const b = block('water-saturation-calculation');
const { waterSaturation: intl } = tKeys.features;
const { shared: sharedIntl } = tKeys;

class WaterSaturationCalculationComponent extends React.Component<Props, IState> {
  ...
  public render() {
    const { researchData: { DEPTH, SW }, calculatedCurvesForTab, appPosition,
              ...
              {calculatedCurvesForTabKeys
                && calculatedCurvesForTabKeys.map((curveName: string, id: number) => {
                  const currentData = cutDataByCore(DEPTH,
                    calculatedCurvesForTab[currentFeatureStateIndex][curveName], SW);
                    ...
                    <Fragment key={`2${id}`}>
                      ...
                      >
                        <Chart
                          ...
                          secondChannelData={coreData.map(item =>
                            (item.isVisible ? item.waterSaturation : -9999))}
                          settings={{
                            header: curveName,
                            lineWidth: 2,
                            dotColor: 'darkcyan',
...
const getIntervalDepthBoundaries = (props: Props): [number, number] => {
  const { researchData: { DEPTH, SW } } = props;
  const currentData = cutDataByCore(DEPTH, DEPTH, SW);
...
const connectedComponent = connect<IStateProps, ActionProps, {}>(mapState,
  mapDispatch)(WaterSaturationCalculationComponent);
const WaterSaturationCalculation = withTranslation()(connectedComponent);

export { WaterSaturationCalculation };
