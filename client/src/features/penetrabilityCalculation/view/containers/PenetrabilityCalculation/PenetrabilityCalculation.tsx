...
import { DIAGRAM_DEFAULT_COLORS, MAX_CURVES_AMOUNT_IN_USERFORTAB_SECTION,
  FIELD_SIZE } from 'shared/constants';
...
import './PenetrabilityCalculation.scss';
...
const b = block('penetrability-calculation');
const { permeability: intl } = tKeys.features;
const { shared: sharedIntl } = tKeys;

class PenetrabilityCalculationComponent extends React.Component<Props, IState> {
  ...
  public render() {
    const { researchData: { DEPTH, PERM }, calculatedCurvesForTab,
      appPosition, verticalScale, coreData, t, setNotification } = this.props;
              ...
              {calculatedCurvesForTabKeys
                && calculatedCurvesForTabKeys.map((curveName: string, id: number) => {
                  const currentData = cutDataByCore(DEPTH,
                    calculatedCurvesForTab[currentFeatureStateIndex][curveName], PERM);
                      ...
                      >
                        <Chart
                          ...
                          secondChannelData={coreData
                            .map(item => (item.isVisible ? item.permability : -9999))}
                          settings={{
                            header: curveName,
                            lineWidth: 2,
                            dotColor: 'black',
  ...
  private onSaveCoreDataItemButtonClick(coreItem: ICoreData): void {
    ...
    const correctedValueByTop = roundedValue < this.intervalMinDepth
      ? this.intervalMinDepth : roundedValue;
    const correctedValueByBottom = correctedValueByTop > this.intervalMaxDepth
      ? this.intervalMaxDepth : correctedValueByTop;
...
const getInitialState = (props: Props): IState => {
  const { researchData: { DEPTH, PERM }, appPosition, featuresStates } = props;
  const currentFeatureStateIndex = getTabIndex(appPosition);
  const currentData = cutDataByCore(DEPTH, DEPTH, PERM);
  let i = 0;
  const coreData = currentData.depth.map((depthValue, index) => ({
    // eslint-disable-next-line no-plusplus
    id: currentData.secondCurve[index] > 0 ? i++ : -1,
    depth: depthValue,
    data: currentData.secondCurve[index],
    isVisible: true,
  }));
  const state = {
    chartsVisibilities: {
...
const getIntervalDepthBoundaries = (props: Props): [number, number] => {
  const { researchData: { DEPTH, PERM } } = props;
  const currentData = cutDataByCore(DEPTH, DEPTH, PERM);
...
const connectedComponent = connect<IStateProps, ActionProps, {}>(mapState,
  mapDispatch)(PenetrabilityCalculationComponent);
const PenetrabilityCalculation = withTranslation()(connectedComponent);

export { PenetrabilityCalculation };
